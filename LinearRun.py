...
square_sig,t=ZeroField.Linear_Square_Wave_Gen(100)

PB=ChargedParticleBunch()
EM=ChargedParticleBunch(electric = np.array([0.035,0,0], dtype=float), magnetic = np.array([0,0,0], dtype=float))
...
for i in range(6):
    while check[0]<=L:
            ...
            time += deltat
            check=Average_Positons[-1]
    while square_sig[int(10*it)]==iter:
            it+=1
...
pd.DataFrame(Average_Positons).to_csv(r'C:\Users\benti\Documents\PHYS 389\100ProtonPositionsLin.csv')
pd.DataFrame(Average_Kinetic_Energies).to_csv(r'C:\Users\benti\Documents\PHYS 389\100ProtonKineticEnergyLinT1.csv')


pd.DataFrame(times).to_csv(r'C:\Users\benti\Documents\PHYS 389\100ProtonTimesLinT1.csv')